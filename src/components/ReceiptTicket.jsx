...
export default function ReceiptTicket({ saleId, store: storeProp, printableId = "receipt-print-area" }) {
  ...
  let itemsGross = 0, itemDiscountTotal = 0, itemsNetSubtotal = 0;
  ...
  const grandTotal = Math.max(0, itemsNetSubtotal - headerDisc + svc + tax);

  return (
    <div className="p-0">
      <style>{`
        @media print { @page { size: 80mm auto; margin: 6mm; } .no-print { display: none !important; } }
        #${printableId} { white-space: normal; word-wrap: break-word; overflow-wrap: break-word; }
      `}</style>
        ...
        <div className="pb-1">
          {renderedItems.map((it) => (
            <div key={it.id ?? `${it.name}-${it.qty}-${it.unitPrice}`} className="mb-1">
              <div className="whitespace-normal break-words leading-tight">{it.name}</div>
              <div className="flex justify-between">
                <span className="text-gray-700">{it.qty} x {fmtIDR(it.unitPrice)}</span>
